...
class RecurrentStaticNets(BaseStaticNets):
    ...
    def __init__(self, config: StaticNetsConfig):
        ...
        for j in range(self.num_layers):
            ...
            layer_j_W_ih: Float[Tensor, "NN LiOS LiIS"] = torch.randn(self.config.num_nets, layer_j_out_size, layer_j_in_size) * layer_j_ih_std
            layer_j_b_ih: Float[Tensor, "NN 1 LiOS"] = torch.randn(self.config.num_nets, 1, layer_j_out_size) * layer_j_ih_std
            layer_j_W_hh: Float[Tensor, "NN LiOS LiOS"] = torch.randn(self.config.num_nets, layer_j_out_size, layer_j_out_size) * layer_j_hh_std
            layer_j_b_hh: Float[Tensor, "NN 1 LiOS"] = torch.randn(self.config.num_nets, 1, layer_j_out_size) * layer_j_hh_std
    ...
    def mutate(self) -> None:
        for j in range(self.num_layers):
            if self.config.sigma_sigma is not None:
                layer_j_xi_W_ih: Float[Tensor, "NN LiOS LiIS"] = torch.randn_like(self.W_ih_weight_sigmas[j]) * self.config.sigma_sigma
                self.W_ih_weight_sigmas[j]: Float[Tensor, "NN LiOS LiIS"] = self.W_ih_weight_sigmas[j] * (1 + layer_j_xi_W_ih)
                layer_j_W_ih_sigma: Float[Tensor, "NN LiOS LiIS"] = self.W_ih_weight_sigmas[j]
                layer_j_xi_b_ih: Float[Tensor, "NN 1 LiOS"] = torch.randn_like(self.W_ih_bias_sigmas[j]) * self.config.sigma_sigma
                self.W_ih_bias_sigmas[j]: Float[Tensor, "NN 1 LiOS"] = self.W_ih_bias_sigmas[j] * (1 + layer_j_xi_b_ih)
                layer_j_b_ih_sigma: Float[Tensor, "NN 1 LiOS"] = self.W_ih_bias_sigmas[j]
                layer_j_xi_W_hh: Float[Tensor, "NN LiOS LiOS"] = torch.randn_like(self.W_hh_weight_sigmas[j]) * self.config.sigma_sigma
                self.W_hh_weight_sigmas[j]: Float[Tensor, "NN LiOS LiOS"] = self.W_hh_weight_sigmas[j] * (1 + layer_j_xi_W_hh)
                layer_j_W_hh_sigma: Float[Tensor, "NN LiOS LiOS"] = self.W_hh_weight_sigmas[j]
                layer_j_xi_b_hh: Float[Tensor, "NN 1 LiOS"] = torch.randn_like(self.W_hh_bias_sigmas[j]) * self.config.sigma_sigma
                self.W_hh_bias_sigmas[j]: Float[Tensor, "NN 1 LiOS"] = self.W_hh_bias_sigmas[j] * (1 + layer_j_xi_b_hh)
                layer_j_b_hh_sigma: Float[Tensor, "NN 1 LiOS"] = self.W_hh_bias_sigmas[j]
            else:
            ...
            self.W_ih_weights[j]: Float[Tensor, "NN LiOS LiIS"] = self.W_ih_weights[j] + torch.randn_like(self.W_ih_weights[j]) * layer_j_W_ih_sigma
            self.W_ih_biases[j]: Float[Tensor, "NN 1 LiOS"] = self.W_ih_biases[j] + torch.randn_like(self.W_ih_biases[j]) * layer_j_b_ih_sigma
    ...
    def __call__(
        ...
        for j in range(self.num_layers):
            ...
            layer_j_ih: Float[Tensor, "NN BS LiOS"] = layer_j_ih + self.W_ih_biases[j]
            layer_j_hh: Float[Tensor, "NN BS LiOS"] = torch.bmm(hidden_states[j], self.W_hh_weights[j].transpose(-1, -2))
            layer_j_hh: Float[Tensor, "NN BS LiOS"] = layer_j_hh + self.W_hh_biases[j]
