...
class DynamicNets(BaseNets):
    ...
    def __init__(self, config: BaseNetsConfig):
        self.config = config
        self.nets: list[Net] = [
            Net(config.num_inputs, config.num_outputs)
            for _ in range(config.num_nets)
        ]
        self._build_computation_tensors()
    ...
    def mutate(self) -> None:
        ...
        self._sync_state_to_nets()
        for net in self.nets:
            net.mutate()
        self._build_computation_tensors()

    def _sync_state_to_nets(self) -> None:
